...
-- extract the stairsminus category from a node name
...
stairsminus.rotate_node_aux = function(itemstack, placer, pointed_thing)
...
stairsminus.register_single = function(category, alternate, info, modname, subname, recipeitem, fields)
	...
	def.groups = stairsminus:prepare_groups(fields.groups)

	if category == "slab" then
		if core.global_exists("place_rotated") then
			def.on_place = place_rotated.slab
		else
			def.on_place = stairsminus.rotate_node_aux
		end
