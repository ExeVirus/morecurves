...
function curve3d.curve2_closed(left_in, right_in, bottomh, toph, bottom_ty, top_ty, g, name)
    g = g or {3,4,5,6,1,2}
    ...
    shapes.curve2d.wall(left, toph-bottomh, top_ty, g[1])
    shapes.curve2d.wall(right, toph-bottomh, top_ty, g[2])
...
--calculate the Bottom and Top
    ...
    shapes.common.quad(bl,tl,tr,br,g[4])
    ...
    shapes.common.quad(br,tr,tl,bl,g[3])
...
--Calculate the front and back
    ...
    shapes.curve2d.curve_segements(left, right, vector.new(0,1,0,0,0,0,0,0), g[5])
    ...
    shapes.curve2d.curve_segements(right, left, vector.new(0,-1,0,0,0,0,0,0), g[6])

    if name ~= "no_export" then export_mesh(name) end
end

function curve3d.point_curve_open(left_point_in, right_in, bottomh, toph, bottom_ty, top_ty, groups, name)
    ...
    --Outer Curve
    shapes.curve2d.wall(right, toph-bottomh, top_ty, groups[4])
    ...
    shapes.curve2d.corner_curve_c_clockwise(left, right, vector.new(0,1,0,0,0,0,0,0), groups[1])
    ...
    shapes.curve2d.corner_curve_clockwise(left, right, vector.new(0,-1,0,0,0,0,0,0), groups[2])

    if name ~= "no_export" then export_mesh(name) end
end

function curve3d.point_curve_closed(left_point_in, right_in, bottomh, toph, bottom_ty, top_ty, groups, name)
    groups = groups or {1,2,3,4,5,6}
    if name ~= "no_export" then reset_mesh() end
    curve3d.point_curve_open(left_point_in, right_in, bottomh, toph, bottom_ty, top_ty, groups, "no_export")
    local left = shapes.util.copy(left_point_in)
    local right = shapes.util.copy(right_in)
