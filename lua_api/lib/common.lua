...
-- Automatically calculates texture coords given bl corner texture coords (tx, ty)
function common.auto_triangle(bl, tl, tr, tx, ty, group, rot)
    local bl, tl, tr = copy(bl), copy(tl), copy(tr)
    ...
    local len_bl_tl, _, _ = vector.length(vector.subtract(tl, bl))
    local len_bl_tr, _, _ = vector.length(vector.subtract(tr, bl))

    tl.tx = bl.tx
    tl.ty = bl.ty + len_bl_tl
    tr.tx = bl.tx + len_bl_tr
    tr.ty = bl.ty

    common.tri(bl, tl, tr, group, rot)
end
...
function common.auto_quad(bl, tl, tr, br, tx, ty, group, rot)
    ...
    local bottom_edge_len, _, _ = vector.length(vector.subtract(br, bl))
    local left_edge_len, _, _ = vector.length(vector.subtract(tl, bl))
